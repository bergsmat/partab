...
If you like, you can write these definitions to a suitable location, and alter them manually. Pass the location of this file using the `metafile` argument to have them included in the parameter table. (Or put it in the model directory as ```<model>.def``` and it will be found automatically.)  *NOTE* Anything that `as.partab` finds in the control stream will take precedent over identically-named items in `metafile`; pass ```ctlmeta = FALSE``` to ignore the control stream metadata.
```{r eval = FALSE}
